...
public partial class v2_invoice : AdminBasePage
{
    ...
    {
        GoBackSalesListWithoutInvoice();
        CommonUtils.SetWebPageTitle("invoice:"+Request.QueryString["id"]);
    }
    /// <summary>
    /// go back to sales list when query id is null or empty
    /// </summary>
    private void GoBackSalesListWithoutInvoice()
    {
        if (Request.QueryString["id"] == "" || Request.QueryString["id"] == null)
        {
            Response.Redirect("./olist.aspx");
        }
    }
	...
	string InvoicePrintHeader(string sType, string sSales, string sNumber, string sDate, string sPO_number, string card_id, string card_name, string supplier_invoice)
	{
		...
		if (title.IndexOf("SALES ORDER") >= 0) // CH 9-2-2011 UPDATED AS CUSTOME REQUIRED, NO HEADER TITLE ON SALES ORDER
			m_branchHeader = "";
		//DEBUG("m_branchHeader",m_branchHeader);
	...
	string InvoicePrintShip(DataRow dr, string shipto)
	{
		...
		string sqlcmd = "select is_web_order from orders where id= '" + orderId + "' ";
		bool is_web_order = false;
		string receiver_name = dr["name"].ToString();
		string receiver_address1 = dr["Address1"].ToString();
		string receiver_address2 = dr["Address2"].ToString();
		string receiver_address3 = dr["Address3"].ToString();
		string receiver_phone = dr["phone"].ToString();
		try
		{
			myCommand = new SqlCommand(sqlcmd);
			myCommand.Connection = myConnection;
			myCommand.Connection.Open();
			is_web_order = (bool)myCommand.ExecuteScalar();
		...
		// Response.Write(is_web_order);
		if (is_web_order)
		{
			sqlcmd = "select * from ShippingInfo where orderId = " + orderId;
			try
			{
				SqlDataAdapter sqlCommand = new SqlDataAdapter(sqlcmd, myConnection);
				int rows = sqlCommand.Fill(dsi, "onlineShipping");
			}
			catch (Exception e)
			{
				ShowExp(sqlcmd, e);
			...
			DataRow onlineRow = dsi.Tables["onlineShipping"].Rows[0];
			receiver_name = onlineRow["receiver"].ToString();
			receiver_address1 = onlineRow["receiver_address1"].ToString();
			receiver_address2 = onlineRow["receiver_address2"].ToString();
			receiver_address3 = onlineRow["receiver_address3"].ToString();
			receiver_phone = onlineRow["receiver_phone"].ToString();
		}
		StringBuilder sb = new StringBuilder();
		...
		if (dr["shipping_method"].ToString() != "1")
		{
			...
			else
			{
				...
				{
					sAddr = receiver_address1;
					sAddr += "<br>";
					sAddr += receiver_address2;
					sAddr += "<br>";
					sAddr += receiver_address3;
				}
				else
				{
				...
				if (is_web_order)
				{
					m_shippingAddress += "<font size=2>" + receiver_name + "</font>" + "<br>";
				...
				if (dr["phone"].ToString() != null && dr["phone"].ToString() != "")
				{
					...
					sb.Append("<br>\r\n");
					m_shippingAddress += "Ph: " + receiver_phone + "<br>";
				}
				if (dr["fax"].ToString() != null && dr["fax"].ToString() != "")
				{
	...
	string BuildInvoice(string sInvoiceNumber)
	{
		...
		DataSet dsi = new DataSet();
		string sc = "SELECT o.id, s.discount_percent, i.customer_gst, b.branch_header, b.branch_footer, i.invoice_number, i.type, i.payment_type, i.commit_date, i.price ";
		sc += ", i.tax, i.total, i.sales, i.cust_ponumber, cr.stock_location, ";
		sc += " i.freight, i.sales_note, c.*, s.code, cr.supplier_code, s.quantity, s.name as item_name, cr.brand, cr.price1, i.branch ";
		sc += ", s.commit_price, s.normal_price, i.type, i.special_shipto, i.shipto, i.shipping_method, ";
		...
		sc += ", '' AS shelf ";
		sc += ", (SELECT name FROM enum WHERE class = 'unit' AND id = cr.unit) AS unit ";
		...
		sc += " LEFT OUTER JOIN card c ON c.id = i.card_id ";// c.barcode=i.barcode ";
	...
	string BuildItemTable(DataTable dt, bool bOrder, string sHeader, string sFooter)
	{
		...
		sb.Append("<td width=8%><b>SupplierCode</td>\r\n");
		if (bOrder)
		...
		{
			sb.Append("<td width=8% align=right><b>PRICE</td>\r\n");
		...
		sb.Append("<tr><td colspan=" + (MyIntParse(m_itemCols) - 2).ToString() + " align=right wrap ><b>Sub-Total :</b></td><td colspan=2  align=right>");
		...
		/*if(dSaveAmount != 0)
		{
			...
			sb.Append("<tr><td colspan=" + (MyIntParse(m_itemCols) - 2).ToString() + " align=right valign=top  wrap>");
			sb.Append("<b>Discount Amount :</b></td><td colspan=2  align=right>" + dSaveAmount.ToString("c") + "</font></td></tr>");
		}*/

		sb.Append("<tr><td colspan=" + (MyIntParse(m_itemCols) - 2).ToString() + " align=right wrap><b>Tax :</b></td><td colspan=2  align=right>");
		sb.Append(dTAX.ToString("c"));
		sb.Append("</td></tr>\r\n");
		m_invoice_total = dAmount;
	...
	string InvoicePrintOneRow(DataRow dr, bool bSystem, bool bIndividual_Price, bool bKit, bool bOrder)
	{
		...
		sb.Append("<td valign='top'>" + code + "</td>");
		if (bService)
		{
			sb.Append("<td>&nbsp;</td>");
			sb.Append("<td>&nbsp;</td>");
			...
			else
			{
				if (m_bDisplayServiceItemWithNoPrice)
				{
					if (bool.Parse(dr["is_service"].ToString()) && double.Parse(dr["commit_price"].ToString()) == 0)
						...
						sb.Append((dDiscountPercent * 100).ToString() + "%");
				}
				else
					sb.Append((dDiscountPercent * 100).ToString() + "%");
			}
		...
		if (!bKit)
		{
			if (m_bDisplayServiceItemWithNoPrice)
			{
				if (bool.Parse(dr["is_service"].ToString()) && double.Parse(dr["commit_price"].ToString()) == 0)
					...
					sb.Append("<td align=right valign=top>" + dr["quantity"].ToString() + " " + dr["unit"].ToString() + "</td>");
			}
			else
				sb.Append("<td align=right valign=top>" + dr["quantity"].ToString() + " " + dr["unit"].ToString() + "</td>");
	...
	string BuildOrderMail(string sOrderNumber)
	{
		...
		string sc = "SELECT  o.id,i.discount_percent, o.customer_gst, b.branch_header,  b.branch_footer, c2.name AS sales, o.system AS iSystem ";
		...
		sc += ", k.kit_id, k.qty AS kit_qty, k.name AS kit_name, k.commit_price AS kit_price, cr.brand ";
