...
public partial class v2_pos : AdminBasePage
{
    ...
    string m_sCheckedDeliveryOption = "1";
    double m_dFreight = 0;
    string sGstNew = "0";
    ...
    protected void Page_Load(object sender, EventArgs eA)
    {
		...
		if (m_orderNumber != "")
		{
			...
			{
				m_tableTitle = "Credit Notes - for invoice #<a title='view invoice details' href='invoice.aspx?i=" + m_invoiceNumber + "' target=_blank><font color=red>" + m_invoiceNumber + "</font></a>";
	...
	protected void Print_Page()
    {
		...
		//DEBUG("ADMIN ",  Session["employee_access_level"].ToString());
		//DEBUG("m_dSetMaximumDiscountPerItem",m_dSetMaximumDiscountPerItem);
		...
		//m_dGstRate = MyDoubleParse(GetSiteSettings("gst_rate_percent", "15")) / 100;						//26.06.2003 NEO	
		int oid = 0;
		if (Request.QueryString["id"] != "")
			oid = MyIntParse(Request.QueryString["id"]);

		double GstRate = GetOrderGst(oid);
		if (Session["custgst" + m_ssid] != null)
			sGstNew = Session["custgst" + m_ssid].ToString();
		else
			sGstNew = GstRate.ToString();

		m_dGstRate = double.Parse(sGstNew);
		//**************** GST Rate New Sean 08/08/2011
		if (Request.QueryString["t"] == "created")
		{
			...
			Response.Write(" value=' Print ' " + Session["button_style"] + "><br><br><br>");
			Response.Write("<input type=button ");
		...
		m_sales = Session["login_card_id"].ToString(); //default
		...
		//item search
		string kwi = g("kwi");
		if (Request.Form["item_code_search"] != null)
			kwi = p("item_code_search");
		if (kwi != "")
		{
			...
			string s_SearchMsg = "";
			s_SearchMsg = DoSerialSearch(kwi);
			//*******************************************************************************
		...
		if (Request.QueryString["t"] == "del")
		{
			...
			else
			{
				...
				else if (DoDeleteOrder())
				{
					...
					Response.Write("<br><br><center><h3>Order/Quote Deleted.</h3>");
		...
		{ // DEBUG ("SEARCH =", Request.QueryString["search"] );
			DoCustomerSearchAndList();
			return;
		}
		...
		else if (Request.Form["cmd"] == "Update Order" || Request.Form["cmd"] == "Update Quote")
		{
			...
			if (DoUpdateOrder())
			{
				if (Request.Form["cmd"] == "Update Order")
					...
					Response.Write("<br><br><center><h3>Quote Updated.</h3>");
				Response.Write("<input type=button  " + Session["button_style"] + "  onclick=window.location=('");
				Response.Write("pos.aspx?id=" + m_orderID + "&ssid=" + m_ssid + "') value='Back to Order List'></center>");
			}
			else
			{
		...
		else if (Request.Form["cmd"] == "Delete Order" || Request.Form["cmd"] == "Delete Quote")
		{
			...
			if (Request.Form["cmd"] == "Delete Quote")
			{
				Response.Write("\\r\\n\\r\\nAre you sure you want to delete this quote?         ");
				Response.Write("\\r\\nThis action cannot be undone.\\r\\n");
				Response.Write("\\r\\nClick OK to delete quote.\\r\\n");
			}
			else
			{
	...
	bool RestoreCustomer()
	{
		...
		Session["pos_branch_id" + m_ssid] = m_branchID;
		Session["pos_sales_id" + m_ssid] = m_sales;
	...
	bool RestoreOrder()
	{
		...
		int items = 0;
		string sc = " SELECT oi.discount_percent, oi.*, o.sales, o.customer_gst ";
		sc += ", (SELECT TOP 1 s.name FROM shelf_item si Join shelf s ON s.id = si.shelf_id WHERE si.code = oi.code ORDER BY si.qty DESC) AS shelf ";
		sc += " FROM order_item oi ";
		sc += " JOIN orders o ON o.id = oi.id ";
		sc += " JOIN code_relations c ON c.code = oi.code ";
		sc += " WHERE oi.id = " + m_orderID + " ";
		sc += " AND kit = 0 ";
		sc += " ORDER BY c.stock_location DESC, shelf DESC, oi.supplier_code DESC ";
		//DEBUG("sc=", sc);
		try
		{
			myAdapter = new SqlDataAdapter(sc, myConnection);
		...
		for (i = 0; i < items; i++)
		{
			...
			if (!AddToCart("", dr["code"].ToString(), dr["supplier"].ToString(), dr["supplier_code"].ToString(),
				...
				dr["item_name"].ToString(), "", dr["discount_percent"].ToString(), dr["pack"].ToString()))
				return false;
			//		DEBUG("added, qty=" + qty + ",code=", dr["code"].ToString());
		}
	...
	bool DoSearchItem(string kw, bool btype)
	{
		...
		if (dst.Tables["isearch"] != null)
			dst.Tables["isearch"].Clear();
		...
		sc += ", moq ";
		/**********************/
		sc += " FROM code_relations p Where 1=1 AND p.skip <>'1' "; // LIKE '"+ kw;

		kw = kw.ToLower();
		if (!btype)
		{
			sc += "  AND (p.supplier_code = '" + kw + "') ";
		}
		else
		{
			if (!TSIsDigit(kw))
			...
			else
			{
				...
				{
					sc += " AND (p.code = (SELECT TOP 1 b.item_code FROM barcode b WHERE b.barcode ='" + kw + "') OR p.code ='" + kw + "' OR LOWER(p.supplier_code) ='" + kw + "' OR p.barcode = '" + kw + "')";
				}
				else
					sc += " AND (Lower(p.supplier_code) = '" + kw + "' OR p.code ='" + kw + "' OR p.barcode = '" + kw + "')";
			}
		}
		//DEBUG(" sc =",sc);
		//return false;
		//	m_customerID = p("customer");
		try
		{
			myAdapter = new SqlDataAdapter(sc, myConnection);
		...
		if (m_nSearchReturn == 1)
		{
			...
			{
				cardtype = DoCustomerType(m_customerID);
				carddis = DoCustomerDis(m_customerID);
				//DEBUG("cardtype",cardtype);	
				//return false;	
			}
			...
			//		{
			//			GetCustomer();
			//			dSalesPrice = MyDoubleParse(dr["price1"].ToString()) * (1-MyDoubleParse(carddis));

			//			Session["discount_per"+m_ssid] = (MyDoubleParse(carddis)*100).ToString();	

			//			if(DoApplyDiscountTotal(m_dGstRate, MyBooleanParse(Session[m_sCompanyName + "gst_onoff_for_pos"].ToString())))
			//				Response.Write("<meta http-equiv=\"refresh\" content=\"0; URL=pos.aspx?ssid=" + m_ssid + "\">");
			//		}
			//		else
			dSalesPrice = GetSalesPriceForDealer(code, "1", Session[m_sCompanyName + "sales_dealer_level_for_pos" + m_ssid].ToString(), m_customerID);
			...
			/*
					if(GetExistItem(code) && g("confirm") != "0")
					{
						...
						Response.Write("{window.location='?a=add&code=" + code + "&supplier_code=" + HttpUtility.UrlEncode(supplier_code) + "&ssid=" + m_ssid + "'}");
			 ...
			 */
			AddToCart(code, supplier, supplier_code, moq, supplier_price, dSalesPrice.ToString(), "", "");
			Response.Write("<meta http-equiv=\"refresh\" content=\"0; URL=" + s_url + "&focus=qty\">");
	...
	string DoCustomerType(string id)
	{
		if (dst.Tables["DoCustomerType"] != null)
			dst.Tables["DoCustomerType"].Clear();
		string CustomerType = "";
		int nRows = 0;
		string sc = "";
		sc += " SELECT type FROM card WHERE 1=1 ";
		sc += " AND id = " + m_customerID;
		//DEBUG("sc1",sc);
		//return "1234";	
		try
		{
			myAdapter = new SqlDataAdapter(sc, myConnection);
			nRows = myAdapter.Fill(dst, "DoCustomerType");
		}
		catch (Exception e)
		{
			ShowExp(sc, e);
		...
		CustomerType = dst.Tables["DoCustomerType"].Rows[0]["type"].ToString();
		return CustomerType;
	}

	string DoCustomerDis(string id)
	{
		if (dst.Tables["DoCustomerDis"] != null)
			dst.Tables["DoCustomerDis"].Clear();
		string CustomerDis = "";
		int nRows = 0;
		string sc = "";
		sc += " SELECT m_discount_rate FROM card WHERE 1=1 ";
		sc += " AND id = " + m_customerID;
		try
		{
			myAdapter = new SqlDataAdapter(sc, myConnection);
			nRows = myAdapter.Fill(dst, "DoCustomerDis");
		}
		catch (Exception e)
		{
			ShowExp(sc, e);
		...
		CustomerDis = dst.Tables["DoCustomerDis"].Rows[0]["m_discount_rate"].ToString();
		return CustomerDis;
	}
	...
	bool DoMPNSearch(string kw)
	{
		...
		kw = kw.ToLower();
		string sc = "SELECT p.code, p.supplier, p.supplier_code, p.name, p.price1, ISNULL(p.supplier_price, 0) AS supplier_price ";
		sc += " FROM code_relations p ";
		sc += " LEFT OUTER JOIN barcode b ON b.item_code= p.code ";
		sc += " WHERE ("; // supplier_code ";// LIKE '"+ kw;
		sc += "  Lower(p.supplier_code) = '" + kw + "'";
		if (m_bUseBarcodeInSalesMode)
			//sc += " OR p.code = (SELECT TOP 1 b.item_code FROM barcode b WHERE b.barcode ='"+ kw +"')";
			sc += " OR Lower(b.barcode) = '" + kw + "'";
		...
		//DEBUG("m_nSearchReturn",m_nSearchReturn.ToString());	
		//return false;
		if (m_nSearchReturn == 1)
		{
			...
			{
				GetCustomer();
				dSalesPrice = MyDoubleParse(dr["price1"].ToString());
			...
			if (GetExistItem(code) && g("confirm") != "0")
			{
				/*			Response.Write("<script ");
							Response.Write(">");
							Response.Write("alert('Item Exists. M_PN : " + supplier_code + "')");
							Response.Write("</script");
							Response.Write(">");
				*/
				Response.Write("<script language=javascript>if(window.confirm('This item is already in your cart, do you want to add it again?'))");
			...
			Response.Write("<meta http-equiv=\"refresh\" content=\"0; URL=pos.aspx?ssid=" + m_ssid + "&focus=qty\">");  //fq, focus qty
	...
	void BindISTable()
	{
		...
		Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:1px;border-style:Solid;border-collapse:collapse;fixed\">");
		Response.Write("<tr style=\"color:white;background-color:#7b90ac;font-weight:bold;\">\r\n");
		...
		Response.Write("<td>SellingPrice(GST Inc)</td>\r\n");
		Response.Write("</tr>\r\n");
	...
	void RestoreAllFields()
	{
		...
		if (Session["pos_branch_id" + m_ssid] != null && Session["pos_branch_id" + m_ssid].ToString() != "")
		{
			m_sBranch = Session["pos_branch_id" + m_ssid].ToString();
			m_branchID = Session["pos_branch_id" + m_ssid].ToString();
		}
		if (Session["pos_sales_id" + m_ssid] != null && Session["pos_sales_id" + m_ssid].ToString() != "")
			m_sSalesID = Session["pos_sales_id" + m_ssid].ToString();
		Session["branch_id"] = m_sBranch;
		//order number
	...
	bool UpdateAllFields()
	{
		...
		m_customerID = Request.Form["customer"].ToString();
		m_sBranch = Request.Form["branch"].ToString();
		m_sSalesID = Request.Form["sales"].ToString();
		...
		Session["sales_freight" + m_ssid] = m_dFreight;
		Session["sales_no_individual_price" + m_ssid] = nip;
		Session["pos_branch_id" + m_ssid] = m_sBranch;
		Session["pos_sales_id" + m_ssid] = m_sSalesID;
		Session["branch_id"] = Session["pos_branch_id" + m_ssid];
	...
	bool PrintCartItemTable()
	{
		...
		Response.Write("<td id='table-list-title-format width=130' align=left width=15%>STOCK ID</td>");

		Response.Write("<td width=70>CODE</td>");
		Response.Write("<td >DESCRIPTION</td>");
		Response.Write("<td align=right>PRICE</td>");
		Response.Write("<td align=right nowrap>DISC(%)</td>");
		Response.Write("<td align=right>STOCK</td>");
		Response.Write("<td align=right>UNIT/QTY</td>");
		...
		for (i = (dtCart.Rows.Count - 1); i >= 0; i--)
		{
			if (dtCart.Rows[i]["site"].ToString() != m_sCompanyName)
				continue;
			...
			//DEBUG("dDiscount =s", dDiscount.ToString());
			double dSalesPrice = 0;
			//////////////set the GST PRICE switch when click on the check box button
			//        DEBUG("dSalesPrice= ", dr["salesPrice"].ToString());
			//		DEBUG("SSS", "SSS");
			try
			{
			...
			//DEBUG("dOldPrice", dOldPrice.ToString());
			// DEBUG("dCurrentPrice", dCurrentPrice.ToString());
			...
			if (m_bOrder)
			{
				SSPrintOneRow(i, dr["code"].ToString(), supplierCode, s_prodName, dsupplierPrice, dSalesPrice, quantity, dRowTotal, dr["s_serialNo"].ToString(), MyDoubleParse(dr["discount_percent"].ToString()), MyBooleanParse(dr["kit"].ToString()), pack);
			...
			{
				SSPrintOneRow(i, dr["code"].ToString(), dr["code"].ToString(), s_prodName, dsupplierPrice, dSalesPrice, quantity, dRowTotal, dr["s_serialNo"].ToString(), MyDoubleParse(dr["discount_percent"].ToString()), MyBooleanParse(dr["kit"].ToString()), pack);
		...
		{
			Response.Write("<script language=javascript>");
			/*		Response.Write(" if(document.all.qty" + (nTotalRows-1).ToString() + ".value == '1'){");
						Response.Write("document.all.qty" + (nTotalRows-1).ToString() + ".focus();  document.all.qty" + (nTotalRows-1).ToString() + ".select();");
					Response.Write("}");
					Response.Write("else{");
		...
		*/
		...
		if (bool.Parse(Session[m_sCompanyName + "gst_onoff_for_pos"].ToString()))
		{
			...
			//	DEBUG("total ", dTotalGST.ToString());
		}
		else
		{
			dTotalGST = (dTotalPrice * m_dGstRate);
			dAmount = dTotalPrice + dTotalGST;
			//	DEBUG("total ", dTotalPrice.ToString());
		}
		...
		//start comment table
		Response.Write("<table align=center><tr><td>&nbsp</td></tr><tr><td><b>&nbsp;Comment : </b></td></tr>");
		Response.Write("<tr><td align=center>");
		Response.Write("<td><textarea name=note  id=textarea2 rows=5 cols=55>");
		Response.Write(HttpUtility.HtmlEncode(m_salesNote));
		Response.Write("</textarea>");
		/*	Response.Write ("<script language='javascript1.2'");
			Response.Write (">");
			Response.Write ("generate_wysiwyg(\'textarea2\')");
			Response.Write ("</script");
			Response.Write (">");
		 */
		Response.Write("</td></tr>");
		...
		//freight	
		Response.Write("<td colspan=3 align=right><b>Freight : </b></td>");
		Response.Write("<td align=right>");
		...
		*/
		if (m_customerID != "" && m_customerID != "0")
		{
			...
			Response.Write("<td  bgcolor=#EEEEE align=right title='Credit Limit : " + dCredit.ToString("c") + ", Account Balance : " + dBalance.ToString("c") + "'>");
			if (bCredit)
			...
			if (MyBooleanParse(GetSiteSettings("enable_credit_term_check", "0")))
			{
				...
				Response.Write("<td  bgcolor=#EEEEE align=right title='Most Overdue Days : " + nOverdueDays.ToString() + "'>");
				if (bTermOK)
					Response.Write("<font color=green>");
				else
					Response.Write("<font color=red>");
		...
		Response.Write("<span style=\"font:bold 12px arail\">Total Amount $ </span><input type=text name=discount_total size=5 maxlenth=8 style=text-align:right onSelect=\"document.all.discount_total_per.value ='' \" onclick=\"document.all.discount_total_per.value ='' \" ");
		if (Session["total_amount" + m_ssid] != null)
			Response.Write(" value='" + Session["total_amount" + m_ssid].ToString() + "'");
		Response.Write("><br>");
		Response.Write("<span style=\"font:bold 12px arail\">Total Percent % </span><input type=text name=discount_total_per size=5 maxlenth=8 style=text-align:right onSelect=\"document.all.discount_total.value ='' \" onclick=\"document.all.discount_total.value ='' \" ");
		...
		Response.Write("<b>" + m_dGstRate.ToString("p") + "");

		//	DEBUG("GST ", m_dGstRate.ToString());
		Response.Write("<b />&nbsp;<select name=custgst >");
		Response.Write("<option value='0.15' ");
		if (m_dGstRate == 0.15)
			Response.Write(" selected ");
		Response.Write("> 15  </option>");
		Response.Write("<option value='0.125' ");
		if (m_dGstRate == 0.125)
			Response.Write(" selected ");
		Response.Write("> 12.5  </option>");
		...
		Response.Write("</td></tr>");// ****************************GST SELECT******************************
									 ...
									 //if(Session["custgst" + m_ssid] != null)
									 //DEBUG("GST",Session["custgst" + m_ssid].ToString());
									 //DEBUG("GST Form",Request.Form["custgst"]);
									 //DEBUG("m_dGstRate", m_dGstRate);


		/*Response.Write("<tr bgcolor=#EEEEE><td colspan=3 align=right>");

		Response.Write("<b>Discount Total(<i>");
		if(!MyBooleanParse(GetSiteSettings("discount_include_GST", "0", true)))
			Response.Write("GST Exclusive");
		else
			Response.Write("GST Inclusive");
		Response.Write("</i>):</b></td><td align=right><input type=text name=discount_total size=5 maxlenth=8 style=text-align:right>");
	...
	//DEBUG("m_broder = ", m_bOrderCreated.ToString());
		...
		//	Response.Write("<tr bgcolor=#ffffff><td colspan=" + (m_cols-1).ToString() + " align=right>");
		Response.Write("<tr><td bgcolor=#EEEEE colspan=3 align=right>");
		Response.Write("<b>Sub-Total : </b></td><td bgcolor=#EEEEE align=right>");
		/*	if(bool.Parse(Session[m_sCompanyName + "gst_onoff_for_pos"].ToString())) 
		...
		*/
		...
		//total amount due
		//Response.Write("<tr bgcolor=#ffffff><td colspan=" + (m_cols-1).ToString() + " align=right>");
		Response.Write("<tr><td  bgcolor=#EEEEE ");
		if (m_customerID != "" && m_customerID != "0")
		...
		Response.Write("<input type=submit name=cmd value='" + Lang("Set") + "' " + Session["button_style"] + "> ");
		Response.Write("</td></tr>");
		...
		{
			//		if(bCredit)
			//		Response.Write("<input type=button "+ Session["button_style"] +" name=cmd value='Stock Record' onclick=\"window.location=('orderstock.aspx?oid="+ m_orderID +"');\">");
			if (m_orderStatus != "2" && m_orderStatus != "3" && m_orderStatus != "6")
			{
				if (m_sSalesType == "quote")
					Response.Write("<input type=submit name=cmd value='Change To Order' " + Session["button_style"] + ">");
				if (m_bEnableCopyOrderFunction)
					Response.Write("<input type=button " + Session["button_style"] + " name=cmd value='Copy " + Capital(m_sSalesType) + "' onclick=\"window.location=('cpso.aspx?oid=" + m_orderID + "');\">");
				Response.Write("<input type=submit " + Session["button_style"] + " name=cmd value='Delete " + Capital(m_sSalesType) + "'>");
				Response.Write("<input type=submit " + Session["button_style"] + " name=cmd value='Update " + Capital(m_sSalesType) + "'>");
	...
	bool SSPrintOneRow(int nRow, string sID, string code, string desc, double dCost, double dPrice, int qty, double dTotal, string sSNnum, double dDiscountPercent, bool bKit, string pack)
	{
		string stock = "";
		if (IsInteger(sID))
		...
		Response.Write(">");
		Response.Write("<input type=hidden name=itemrows" + nRow.ToString() + " value='" + nRow.ToString() + "'>");
		//PACK
		Response.Write("<td><input type =text size=5 name=pack" + nRow.ToString() + " value='" + pack + "'>");
		...
		if (CheckPatent("viewsales"))
		{
			...
			Response.Write("<input type=button title='View Customer Sales History' onclick=\"javascript:viewsales_window=window.open('viewcustsales.aspx?");
		...
		Response.Write(" value='" + dDiscountPercent.ToString() + "' ");
		Response.Write(" onchange=\"return iCalPrice(document.form1.price" + nRow + ".value, document.form1.qty" + nRow + ".value, " + nRow + ", document.form1.discount" + nRow + ".value);\" ");
		//if(bKit)
		...
		//quantity
		Response.Write("<td align=right nowrap>" + GetItemUnit(sID) + " <input type=text size=3 autocomplete=off style='text-align:right' name=qty" + nRow.ToString());
	...
	bool PrintShipToTable()
	{
		...
		Response.Write("<b>Bill To : <br></b><td><td valign=top>");
		...
		if (!bCashSales)
		{
			...
			//		sCompany += " "+ dr["last_name"].ToString();
		...
		Response.Write("<b>Shipping Method :</b></td><td>");
		Response.Write("<select name=shipping_method onchange=\"OnShippingMethodChange();\">");
		if (bCashSales)
			Response.Write("<option value=1>PICK UP</option>");
		else
			Response.Write(GetEnumOptions("shipping_method", m_nShippingMethod, false, true));//, m_sCheckedDeliveryOption));
		Response.Write("</select>");
		Response.Write("</td></tr>");
		...
		Response.Write("Pick Up Time : </td><td><input type=text size=10 name=pickup_time maxlength=49 value=\"" + m_pickupTime + "\">");
		Response.Write("</td></tr></table>");
		//end pick up row
		Response.Write("</td></tr></table>");
		//end ****
		...
		*/
		...
		Response.Write("<a class=o style=\"cursor:hand\" onclick=\"window.open('cardaddr.aspx?t=sa&cid=" + m_customerID + "');\">" + Lang("Select Address") + "</a>");
	...
	bool PrintSalesHeaderTable(string sCustomerPONumber)
	{
		...
		Response.Write("<b>Customer : </b></td><td>");
		...
		Response.Write("</select>");
		Response.Write(" <input type=button title='add new customer' value='EZ CARD' onclick=\"javascript:addcard_window=window.open('ezcard.aspx?r=" + DateTime.Now.ToOADate() + "&luri=" + HttpUtility.UrlEncode(luri) + "','','resizable=1, screenX=300,screenY=200,left=300,top=200'); addcard_window.focus();\"  " + Session["button_style"] + ">");
		if (m_customerID.Length > 2)
		{
			Response.Write("<input type=button onclick=\"javascript:viewcard_window=window.open('viewcard.aspx?");
			Response.Write("id=" + m_customerID + "','','width=350,height=340, scrollbars=1, resizable=1');\" value='View Card' " + Session["button_style"] + ">");
		}
		...
		Response.Write("<tr><td>");
		Response.Write("<b>P.O.Number : </b></td><td><input type=editbox name=custpo value='" + sCustomerPONumber + "'>");
		Response.Write("</td></tr>");
		...
		if (Session["branch_support"] != null)
		{
			...
			if (!PrintBranchNameOptions(m_branchID))
				return false;
			Response.Write("</tr>");
		}
		else
		...
		//DEBUG("sdfs =", Session["pos_branch_id" + m_ssid].ToString());
		...
		Response.Write("</td></tr>");
		Response.Write("<tr><td colspan=1 align=right><b>No Individual Price : </b></td><td><input type=checkbox name=nip ");
		if (m_bNoIndividualPrice)
			Response.Write(" checked");
		Response.Write(">");
		Response.Write("</td></tr>");
		...
		Response.Write("<tr><td colspan=1 align=right><b>Show Price in GST Inc : </b></td><td><input type=checkbox name=gst ");
		if (bool.Parse(Session[m_sCompanyName + "gst_onoff_for_pos"].ToString()))
			Response.Write(" checked");
	...
	void PrintSalesPersonOptions(string current)
	{
		...
		if (Session["branch_support"] != null && Session["employee_access_level"].ToString() != "10")
		{
	...
	bool DoCustomerSearchAndList()
	{
		...
		Response.Write("<td width='17' height='30' id='top-header1'>&nbsp;</td>");
		Response.Write("<td height='30' class='pageName' id='top-header2' ><font size=3><font size=+1><b>Search for Customer</b><font color=red><b>");
		Response.Write("</td>");
		Response.Write("<td width='76' id='top-header3'>&nbsp;</td>");
		Response.Write("  <td  height='30' id='top-header4'>&nbsp;</td>");
		Response.Write("<td width='40' id='top-header5'>&nbsp;</td>");
		...
		Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:1px;border-style:Solid;border-collapse:collapse;fixed\">");
		Response.Write("<tr><td>");
		Response.Write("<input type=editbox size=25 name=ckw value='" + kw_record + "'>");
		...
		Response.Write("<input type=button onclick=window.location=('pos.aspx?ci=0&ssid=" + m_ssid + "') value='Cash Sales' " + Session["button_style"] + ">");
	...
	bool CreateOrder(string branch_id, string card_id, string po_number, string special_shipto, string shipto,
					 ...
					 bool bNoIndividualPrice, ref string order_number, double gst_selection)
	{
		string reason = "";
		...
		//	if(!CheckBottomPrice())
		//		return false;
		string myFreight = Request.Form["freight"];
		...
		sc += " INSERT INTO orders (number, type, card_id, po_number, freight) "; //, sales_manager) ";
		...
		try
		{
			...
			if (myCommand1.Fill(dsco, "id") == 1)
			{
				...
				sc = "UPDATE orders SET number=" + m_orderNumber + ", branch=" + branch_id + ", sales_note = N'" + sales_note + "' ";
				if (special_shipto == "1")
	...
	bool DoCreateOrder(bool bSystem, string sCustPONumber, string sSalesNote)
	{
		...
		return CreateOrder(branch, m_customerID, sCustPONumber, m_specialShipto, m_specialShiptoAddr, m_nShippingMethod,
			m_pickupTime, contact, m_sales, EncodeQuote(sSalesNote), m_bNoIndividualPrice, ref m_orderID, m_dGstRate);
	...
	bool CheckBottomPrice()
	{
		...
		for (int i = 0; i < dtCart.Rows.Count; i++)
		{
			...
			{
				Response.Write("<br><br><center><h3>Product not found : " + code);
				return false;
			}
			...
			double dAvgCostNZD = Math.Round(MyMoneyParse(GetAverageCost(drp["code"].ToString()).ToString()), 2);
			if (MyBooleanParse(GetSiteSettings("enable_under_cost_checked", "1", true)))
			{
				...
				if (iQty > 0 && dPriceCheck < dAvgCostNZD)
				{
					//				 
					//				 
					string s = "";
					s += "Warning, Under Cost sales detected:";
					s += "\\r\\nProduct Code : " + code;
					s += "\\r\\nDescription : " + drp["name"].ToString();
					s += "\\r\\nLast Cost " + m_sCurrencyName + " : " + dLastCostNZD.ToString("c");
					s += "\\r\\nManual Cost " + m_sCurrencyName + " : " + dManualCostNZD.ToString("c");
					s += "\\r\\nSales Price : " + dPriceCheck.ToString("c");
					s += "\\r\\n Do you want to continue?\\r\\n";
					string sj = "<script language=javascript>";
					sj += "if(!window.confirm('" + s + "')){history.go(-1);}";
					sj += "</script";
					sj += ">";
					Response.Write(sj);
					return true;
	...
	bool DoUpdateOrder()
	{
		...
		sc += ", sales_note = N'" + EncodeQuote(m_salesNote) + "' ";
		sc += ", sales=" + salesID;
		...
		sc += ", no_individual_price = " + nip;
		sc += ", customer_gst = '" + Request.Form["custgst"] + "'";
		sc += " WHERE id=" + m_orderID;
	...
	bool WriteOrderItems(string order_id)
	{
		...
		for (int i = 0; i < dtCart.Rows.Count; i++)
		{
			...
			sc += ", commit_price, discount_percent, pack ) VALUES(" + order_id + ", " + dr["code"].ToString() + ", ";
			sc += dr["quantity"].ToString() + ", N'" + name + "', '" + dr["supplier"].ToString();
			sc += "', '" + dr["supplier_code"].ToString() + "', " + Math.Round(MyMoneyParse(dr["supplierPrice"].ToString()), 2);
			sc += " , " + dPrice + " ";
			sc += ", " + dr["discount_percent"].ToString() + "";
			sc += ", N'" + Request.Form["pack" + i].ToString() + "'";
			sc += " ) ";
			...
			if (drp["specials"].ToString() != "-1")
			{
				if (drp["special_price_end_date"].ToString() != null && drp["special_price_end_date"].ToString() != "")
				{
					...
					if (check_time <= special_end)
						dPrice = MyDoubleParse(drp["special_price"].ToString()) / (1 + Double.Parse(Session[m_sCompanyName + "gst_rate"].ToString()));
				}
			}
			else if (sKit == "0") ///it's kit then get sales dealer price
	...
	string AssignPackWithKid(string code, string id)
	{
		if (dst.Tables["pack"] != null)
			dst.Tables["pack"].Clear();
		string sc = " SELECT kid FROM order_item WHERE id = " + id + " AND supplier_code = '" + code + "' ";
		//DEBUG("SC", sc);
		try
		{
			myAdapter = new SqlDataAdapter(sc, myConnection);
			if (myAdapter.Fill(dst, "pack") <= 0)
				return "";
		}
		catch (Exception ex)
		{
		...
		return dst.Tables["pack"].Rows[0]["kid"].ToString();
	}
	double GetAverageCost(string code)
	{
		if (dst.Tables["avgcost"] != null)
			dst.Tables["avgcost"].Clear();
		string sc = " SELECT average_cost FROM code_relations WHERE code='" + code + "'";
		try
		{
			myAdapter = new SqlDataAdapter(sc, myConnection);
			if (myAdapter.Fill(dst, "avgcost") <= 0)
				return 0;
		}
		catch (Exception ex)
		{
		...
		return double.Parse(dst.Tables["avgcost"].Rows[0]["average_cost"].ToString());
	}

	bool GetExistItem(string code)
	{
		if (Session["ShoppingCart" + m_ssid] == null)
			return true;

		DataTable dtblCart = (DataTable)Session["ShoppingCart" + m_ssid];
		//if(dtCart != null)
		//	dtCart.Clear();
		//dtCart.AcceptChanges(); //Commits all the changes made to this row since the last time AcceptChanges was called
		int rows = dtblCart.Rows.Count;
		for (int i = 0; i < rows; i++)
		{
			//DEBUG("CODE 1", dtblCart.Rows[i]["code"].ToString() + " CODE 2 " + code);
			//return false;
			if (code == dtblCart.Rows[i]["code"].ToString())
				return true;
	...
	double GetOrderGst(int id)
	{

		if (dst.Tables["gst"] != null)
			dst.Tables["gst"].Clear();
		string sc = " SELECT customer_gst FROM orders WHERE id='" + id + "'";
		try
		{
			myAdapter = new SqlDataAdapter(sc, myConnection);
			if (myAdapter.Fill(dst, "gst") <= 0)
				return m_sSysGST();
		}
		catch (Exception ex)
		{
		...
		return double.Parse(dst.Tables["gst"].Rows[0]["customer_gst"].ToString());
	}
 void BindGrid()
	{
	...
	public void MyDataGrid_PageA(object sender, DataGridPageChangedEventArgs e)
	{
